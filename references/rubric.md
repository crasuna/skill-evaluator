...
`rubric 综合分` 必须由下列 8 个分项相加得到，不允许只给总分。报告必须包含 `rubric 分项`，逐项写成 `分类名：x/y - 一句扣分理由`；满分项写 `无扣分。`
...
- 2：可选步骤、执行顺序、tie-breaker、输出或评分锚点和人工判断阈值明确；评估类目标技能必须输出分项依据以降低分数漂移。
...
rubric 分项
- 触发元数据：15/15 - 无扣分。
- 工作流清晰度：13/15 - 输出边界说明仍需更明确。
- 渐进披露：15/15 - 无扣分。
- 资源设计：14/15 - fixture 覆盖略少。
- 验证：8/10 - 未说明何时运行 forward-test。
- 安全与范围：10/10 - 无扣分。
- 实践差异风险：6/10 - 内容边界不清，可能导致不同 Markdown 输出。
- 卫生：7/10 - 示例验证命令不够完整。
